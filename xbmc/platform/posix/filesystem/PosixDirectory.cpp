...
bool CPosixDirectory::GetDirectory(const CURL& url, CFileItemList &items)
{
  ...
  {
    const std::string& strName = entry->d_name;

    if (std::ranges::find(exclusions, strName) == notExcluded)
    {
      std::string itemPath(root + strName);
      std::string itemLabel(strName);
      CCharsetConverter::unknownToUTF8(itemLabel);
      CFileItemPtr& pItem = fileItems.emplace_back(std::make_shared<CFileItem>(itemLabel));
          ...
          !(m_flags & DIR_FLAG_NO_FILE_INFO))
      {
        if (stat(itemPath.c_str(), &buffer) == 0)
        {
          ...
          pItem->SetSize(isDir ? 0 : buffer.st_size);
        }
      }
      ...
      if (StringUtils::StartsWith(strName, "."))
