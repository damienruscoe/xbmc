...
bool CNFSDirectory::GetDirectory(const CURL& url, CFileItemList &items)
{
  ...
  {
    const std::string& strName = dirent->name;

    //resolve symlinks
    //resolve symlink changes dirent and strName
    ...
    if (std::ranges::find(exclusions, strName) == notExcluded)
    {
      ...
      std::string path = isSymLink ? resolvedPath : (myStrPath + strName);
      if (isDir)
        URIUtils::AddSlashAtEnd(path);

      CFileItemPtr& item = fileItems.emplace_back(std::make_shared<CFileItem>(strName));
      item->SetPath(path);
