...
bool IsAudio(const CFileItem& item)
{
  ...
  return item.GetURLRef().HasExtension(
...
{
  return item.GetURLRef().IsCDDA();
}

bool IsCUESheet(const CFileItem& item)
{
  return item.GetURLRef().HasExtension(".cue");
}

bool IsLyrics(const CFileItem& item)
{
  return item.GetURLRef().HasExtension(".cdg|.lrc");
}

bool IsMusicDb(const CFileItem& item)
{
  return item.GetURLRef().IsMusicDb();
}
